...
export const IcecastConfigForm: React.FC<IcecastConfigFormProps> = ({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 text-sm text-[var(--text-primary)]">
      {/* ── Section 1: Identity & Engine ── */}
      <div className="bg-[var(--input-bg)]/40 p-4 rounded-xl border border-[var(--glass-border)] space-y-4">
        <div className="flex items-center gap-2 text-brand-lime font-bold uppercase text-xs tracking-wider border-b border-[var(--glass-border)] pb-2">
          <span>📻</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              className="w-full bg-[var(--input-bg)] border border-[var(--glass-border)] rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brand-lime"
      ...
      <div className="bg-[var(--input-bg)]/40 p-4 rounded-xl border border-[var(--glass-border)] space-y-4">
        <div className="flex items-center justify-between border-b border-[var(--glass-border)] pb-2">
          <div className="flex items-center gap-2 text-brand-lime font-bold uppercase text-xs tracking-wider">
            <span>🌐</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className={`p-3 rounded-lg border ${httpEnabled ? 'border-[var(--glass-border)] bg-[var(--input-bg)]' : 'border-dashed border-[var(--glass-border)]/50 opacity-60'}`}>
            ...
            <input
              ...
              className="w-full bg-[var(--bg-card)] border border-[var(--glass-border)] rounded-lg px-3 py-1.5 text-sm font-mono focus:outline-none focus:border-brand-lime"
              placeholder="7000"
            />
          </div>

          <div className={`p-3 rounded-lg border ${sslEnabled ? 'border-brand-lime/40 bg-brand-lime/5' : 'border-dashed border-[var(--glass-border)]/50 opacity-60'}`}>
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-bold uppercase text-[var(--text-primary)] flex items-center gap-1">
                <ShieldIcon size={14} className="text-brand-lime" />
        ...
        {sslEnabled && !hasCertificates && (
          <div className="bg-amber-500/10 border border-amber-500/30 text-amber-300 text-xs p-3 rounded-lg flex items-center gap-2">
            <span>⚠️</span>
      ...
      <div className="bg-[var(--input-bg)]/40 p-4 rounded-xl border border-[var(--glass-border)] space-y-4">
        <div className="flex items-center justify-between border-b border-[var(--glass-border)] pb-2">
          ...
          <button
            ...
            className="text-xs text-[var(--text-secondary)] hover:text-brand-lime transition-colors"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-[var(--text-secondary)] mb-1">
              {t('services.icecast.sourcePassword', 'Contraseña de Emisión (Source Password)')} *
            </label>
            <input
              ...
              className="w-full bg-[var(--input-bg)] border border-[var(--glass-border)] rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:border-brand-lime"
              placeholder="hackme"
            />
            <span className="text-[11px] text-[var(--text-secondary)]">
              {t('services.icecast.sourcePassHelp', 'Utilizada por FFmpeg, Butt, Mixxx u otros emisores para inyectar flujos.')}
            </span>
          </div>

          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-[var(--text-secondary)] mb-1">
              {t('services.icecast.relayPassword', 'Contraseña de Reenvío (Relay Password)')}
            </label>
            <input
              type={showPasswords ? 'text' : 'password'}
              value={relayPassword}
          ...
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-[var(--text-secondary)] mb-1">
              {t('services.icecast.adminUser', 'Usuario Administrador Web')}
            </label>
            <input
              type="text"
              required
          ...
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-[var(--text-secondary)] mb-1">
              {t('services.icecast.adminPassword', 'Contraseña Administrador Web')} *
            </label>
            <input
              type={showPasswords ? 'text' : 'password'}
              required
      ...
      <div className="bg-[var(--input-bg)]/40 p-4 rounded-xl border border-[var(--glass-border)] space-y-4">
        <div className="flex items-center justify-between border-b border-[var(--glass-border)] pb-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1 bg-brand-lime/10 border border-brand-lime/30 text-brand-lime rounded-lg text-xs font-bold uppercase tracking-wider hover:bg-brand-lime/20 transition-colors"
          >
            <PlusIcon size={14} />
        ...
        {mounts.length === 0 ? (
          <div className="text-center py-6 text-[var(--text-secondary)] text-xs border border-dashed border-[var(--glass-border)] rounded-lg">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-[var(--glass-border)] text-xs text-[var(--text-secondary)] uppercase">
                  <th className="py-2 px-3 font-semibold">{t('services.icecast.mountNameCol', 'Punto de Montaje')}</th>
                  <th className="py-2 px-3 font-semibold">{t('services.icecast.maxListenersCol', 'Max Oyentes')}</th>
                  <th className="py-2 px-3 font-semibold">{t('services.icecast.fallbackCol', 'Fallback de Respaldo')}</th>
                  <th className="py-2 px-3 font-semibold">{t('services.icecast.burstCol', 'Burst (Búfer)')}</th>
                  <th className="py-2 px-3 text-right font-semibold">{t('common.actions', 'Acciones')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[var(--glass-border)]">
                {mounts.map((m, idx) => (
                  <tr key={idx} className="hover:bg-[var(--bg-card)]/50 transition-colors text-xs">
                    <td className="py-2.5 px-3 font-mono font-bold text-brand-lime">
                      {m.mount_name}
                    </td>
                    ...
                    <td className="py-2.5 px-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          className="text-[var(--text-secondary)] hover:text-brand-lime p-1"
                          title="Editar"
                        >
                          <PencilIcon size={14} />
                        </button>
      ...
      <div className="flex items-center justify-end gap-3 pt-4 border-t border-[var(--glass-border)]">
        <button
          type="button"
          onClick={onCancel}
          className="px-4 py-2 rounded-lg border border-[var(--glass-border)] text-xs font-bold uppercase tracking-wider text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--input-bg)] transition-colors"
        >
        ...
        <button
          ...
          className="px-5 py-2 bg-brand-lime text-black rounded-lg text-xs font-bold uppercase tracking-wider hover:brightness-110 active:scale-95 transition-all shadow-md flex items-center gap-2 disabled:opacity-50"
        >
          <CheckIcon size={16} />
      ...
      {mountModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-[var(--bg-card)] border border-[var(--glass-border)] rounded-xl w-full max-w-md p-5 space-y-4 shadow-2xl">
            <h4 className="text-sm font-bold uppercase text-brand-lime tracking-wide">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-[var(--input-bg)] border border-[var(--glass-border)] rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-brand-lime"
            ...
            <div className="flex items-center gap-2 pt-1">
              <input
                ...
                className="accent-brand-lime"
              />
              <label htmlFor="fallback_override" className="text-xs text-[var(--text-secondary)] cursor-pointer">
            ...
            <div className="flex items-center justify-end gap-2 pt-3 border-t border-[var(--glass-border)]">
              <button
                ...
                className="px-3 py-1.5 rounded-lg border border-[var(--glass-border)] text-xs uppercase font-bold text-[var(--text-secondary)]"
              >
                {t('common.cancel', 'Cancelar')}
              </button>
              <button
                ...
                className="px-4 py-1.5 bg-brand-lime text-black rounded-lg text-xs uppercase font-bold disabled:opacity-50"
              >
