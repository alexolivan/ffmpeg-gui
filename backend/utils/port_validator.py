...
def extract_ports_from_service(
    ...
    Returns tuples of (port_number, port_label, service_name, service_id).
    """
    ports = []
    cfg = config or {}
    ...
    if s_type == "mediamtx_hub":
        ...
        if mtx.get("rtsp_enabled", True):
            ...
            if mtx.get("rtp_port"):
                try: ports.append((int(mtx["rtp_port"]), "RTP (RTSP UDP)", service_name, service_id))
                except (ValueError, TypeError): pass
            else:
                ports.append((8000, "RTP (RTSP UDP Default)", service_name, service_id))
            if mtx.get("rtcp_port"):
                try: ports.append((int(mtx["rtcp_port"]), "RTCP (RTSP UDP)", service_name, service_id))
                except (ValueError, TypeError): pass
            else:
        ...
        if mtx.get("api_enabled", True):
            ...
            try: ports.append((int(api_p), "MediaMTX Control API", service_name, service_id))
            except (ValueError, TypeError): pass

    elif s_type == "icecast_server":
        ice = cfg.get("icecast_config", cfg)
        if ice.get("port"):
            try: ports.append((int(ice["port"]), "Icecast HTTP", service_name, service_id))
            except (ValueError, TypeError): pass
        ...
        for inp in inputs:
            ...
            if port:
                try:
                    p_num = int(port)
                    # Listener mode inputs take the local port
                    if mode == "listener" or inp_type in ["udp", "rtp", "tcp", "http"]:
                        ports.append((p_num, f"FFmpeg In ({inp_type.upper()})", service_name, service_id))
        ...
        for out in outputs:
            if isinstance(out, dict) and out.get("mode") == "listener" and out.get("port"):
                try:
                    ports.append((int(out["port"]), f"FFmpeg Out Listener ({out.get('type', '').upper()})", service_name, service_id))
                except (ValueError, TypeError):
                    pass
...
def validate_service_port_conflicts(
    ...
    1. The GUI web panel port.
    2. Any port already allocated to another service in SQLite.
    ...
    for port, label, _, _ in target_ports:
        if port in gui_reserved:
            raise HTTPException(
                status_code=400,
                detail=f"Port {port} ({label}) is reserved for the ffmpeg-gui web panel."
            )

    # Check internal collisions within the same service payload
    seen_in_payload = {}
    for port, label, _, _ in target_ports:
        if port in seen_in_payload:
            raise HTTPException(
                status_code=400,
                detail=f"Internal port conflict: Port {port} is defined multiple times ({seen_in_payload[port]} and {label})."
            )
        seen_in_payload[port] = label
    ...
    for other in other_services:
        ...
        for other_p, other_label, other_name, other_id in other_ports:
            for port, label, _, _ in target_ports:
                if port == other_p:
                    raise HTTPException(
                        status_code=400,
                        detail=(
                            f"Port collision: Port {port} ({label}) is already in use by "
                            f"service '{other_name}' (ID: {other_id}, {other_label})."
                        )
                    )
...
def get_next_available_mediamtx_ports(db: Session, exclude_service_id: Optional[int] = None) -> Dict[str, int]:
    ...
    Scans existing services and increments offsets until all 8 ports are conflict-free.
    """
    gui_reserved = get_gui_reserved_ports()
    other_services = db.query(Service).filter(Service.id != exclude_service_id).all() if exclude_service_id else db.query(Service).all()
    
    occupied_ports = set(gui_reserved)
    for other in other_services:
        for p, _, _, _ in extract_ports_from_service(
            service_id=other.id,
            service_name=other.name,
            service_type=getattr(other, "service_type", "ffmpeg_stream"),
            config=other.config,
